...
export default {
  ...
  methods: {
    ...
    verifyPayment(){
      axios({url: `${baseURL}/hmo/payment/bulk/verify`, data:{'reference': this.paystackData.reference}, method: 'POST'})
      .then(res=>{
          this.$store.commit('endLoading')
    ...
    payNow(){
      ...
      axios({url: `${baseURL}/hmo/payment/bulk/init`, data: data, method: 'POST'})
      .then(res=>{
        ...
        this.paystackData.public_key = res.data.data.public_key
                ...
                this.paystackData.transaction_charge = res.data.data.flatfee
